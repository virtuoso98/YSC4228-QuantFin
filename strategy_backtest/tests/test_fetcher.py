...
def test_invalid_strategy_param():
    for _ in range(50):
        ...
        with pytest.raises(ValueError) as excinfo:
            ...
            assert "--strategy_type: Only 'R'(Reversal) or 'M'(Momentum)" \
                 in excinfo.value


def test_valid_strategy_param():
    for valid_strat in ['R', 'M']:
        input_args = {
            "tickers": ["AAPL", "TSLA", "GOOG", "NVDA"],
            "b": "20180104",
            "e": "20190403",
            "initial_aum": 10 ** 6,
            "strategy_type": valid_strat,
            "days": 30,
            "top_pct": 40
        }
